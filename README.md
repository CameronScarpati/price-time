...
Bitstamp's BTC/USD order book. Buyers in blue press up from below, sellers in
amber press down from above, and between them a living gap, the spread, widens
and knits back together dozens of times a minute. Nothing on screen is
decoration. Every mark is caused by a real order arriving, waiting, cancelling,
or filling, reconstructed order-by-order and run through a matching engine
...
cancelled, not filled. The line itself (who is ahead of whom, how long they
have waited, who gets paid) is where the market's texture lives, and it is
...
used when a device should not stream), and **simulated**.
...
- The venue snapshot does not carry order creation times, so ages are known
  only from arrival onward; a freshly seeded book starts its clocks at zero.
- Stops, icebergs-as-certainty, pegs, and hidden orders are not shown: the
